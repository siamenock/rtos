...
void ni_init0() {
	...
	ni_port = gmalloc(sizeof(int*) * nics_count);
}
...
dropped:
	...
	if(dmac & ETHER_MULTICAST) {
		...
		while(map_iterator_has_next(&iter)) {
			...
			if(ni->port == ni_port[nic_index][local_port]) {
				if(input(ni))
					ni_rx++;
			}
		}
		...
		if(ni && ni->port == ni_port[nic_index][local_port]) {
			if(input(ni))
				ni_rx++;
		}
	}
