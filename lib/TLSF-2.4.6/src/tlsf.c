...
/******************************************************************/
size_t init_memory_pool(size_t mem_pool_size, void *mem_pool, int use_lock)
{
/******************************************************************/
    tlsf_t *tlsf;
    bhdr_t *b, *ib;
    ...
    // TODO: Fix the TLSF's bug in the case of reusing memory pool
    /*
    ...
    */
    ...
    bzero(mem_pool, sizeof(tlsf_t));

    tlsf->tlsf_signature = TLSF_SIGNATURE;

    tlsf->use_lock = use_lock;
...
/******************************************************************/
    ...
    #if TLSF_STATISTIC
    {
    bhdr_t * b = (bhdr_t *) ((char *) b0->ptr.buffer - BHDR_OVERHEAD);
    TLSF_ADD_SIZE(tlsf, b);
    tlsf->total_size += area_size;
    }
    #endif
...
/******************************************************************/
static void *_malloc_ex(size_t size, void *mem_pool)
{
/******************************************************************/
    tlsf_t *tlsf = (tlsf_t *) mem_pool;
    bhdr_t *b, *b2, *next_b;
...
void *malloc_ex(size_t size, void *mem_pool) {
    if(((tlsf_t*)mem_pool)->use_lock)
        TLSF_ACQUIRE_LOCK(&((tlsf_t *)mem_pool)->lock);

    void* ptr = _malloc_ex(size, mem_pool);
...
/******************************************************************/
static void _free_ex(void *ptr, void *mem_pool)
{
/******************************************************************/
    tlsf_t *tlsf = (tlsf_t *) mem_pool;
    bhdr_t *b, *tmp_b;
...
/******************************************************************/
static void *_realloc_ex(void *ptr, size_t new_size, void *mem_pool)
{
/******************************************************************/
    tlsf_t *tlsf = (tlsf_t *) mem_pool;
    void *ptr_aux;
...
void *realloc_ex(void *ptr, size_t new_size, void *mem_pool) {
    ...
    ptr = _realloc_ex(ptr, new_size, mem_pool);
...
/******************************************************************/
static void *_calloc_ex(size_t nelem, size_t elem_size, void *mem_pool)
{
/******************************************************************/
    void *ptr;
...
void *calloc_ex(size_t nelem, size_t elem_size, void *mem_pool) {
    ...
    void* ptr = _calloc_ex(nelem, elem_size, mem_pool);
