...
{
	//int32 ret;
	DiskDriver* disk_driver = pdir->ptffs->driver->driver;
	...
	if (((ret = cache_readsector(pdir->ptffs->pcache, clus2sec(pdir->ptffs, pdir->cur_clus) + pdir->cur_sec, pdir->secbuf)) == CACHE_OK)) {
	End of GurumNetworks modification */

	boot_sector_t* pbs = pdir->ptffs->pbs;
	size_t size = pbs->byts_per_sec * pbs->sec_per_clus;
	uint32_t sector = clus2sec(pdir->ptffs, pdir->cur_clus) + pdir->cur_sec;
	void* data = cache_get(pdir->ptffs->driver->cache, (void*)(uintptr_t)sector);
	if(!data) {
		data = gmalloc(size);
		if(disk_driver->read(disk_driver, sector, pbs->sec_per_clus, data) < 0) {
			gfree(data);
			DBG("%s : disk read error\n", __FUNCTION__);
			return ERR_DIR_INVALID_DEVICE;
		}

		if(!cache_set(pdir->ptffs->driver->cache, (void*)(uintptr_t)sector, data)) {
			gfree(data);
			DBG("%s : cache setting error\n", __FUNCTION__);
			return ERR_DIR_INVALID_DEVICE;
		}
	}
	
	memcpy(pdir->secbuf, data, size);
	...
	IN	tdir_entry_t * pdir_entry)
{
	...
	if (ret == DIRENTRY_OK) {
		...
		if (pdirent->dir_name[0] == 0x00) {
			...
			for (di = 1; di < pdir_entry->dirent_num; di++) {
				if ((ret = dirent_find_free_entry(pdir, pdir_entry)) == DIRENTRY_OK) {
					...
					memcpy(pdirent, pdir_entry->pdirent + di, sizeof(dir_entry_t));
					pdir->cur_dir_entry++;
				}
				else {
					DBG("%s(): Get free entry error.\n", __FUNCTION__);
					break;
				}
			}
			...
			memcpy(pdirent, pdir_entry->pdirent, pdir_entry->dirent_num * sizeof(dir_entry_t));
		...
		DBG("%s(): No free clusters, probably disk is full.\n", __FUNCTION__);
...
#ifdef _KERNEL_
	ASSERT(!memcmp(pdst_entry[pdir_entry->dirent_num - 1].dir_name, pdir_entry->pdirent[pdir_entry->dirent_num - 1].dir_name, 11));
#endif

	memcpy(pdst_entry, pdir_entry->pdirent, pdir_entry->dirent_num * sizeof(dir_entry_t));
	...
	OUT	tdir_entry_t * pdir_entry)
{
	...
	secbuf = (ubyte *)malloc(pbs->byts_per_sec * pbs->sec_per_clus);
	memset(secbuf, 0, pbs->byts_per_sec * pbs->sec_per_clus);
	...
	IN	byte * path)
{
	...
	for (itoken = 0; itoken < ntoken; itoken++) {
		...
		if (!(dirent_get_dir_attr(pdir_entry) & ATTR_DIRECTORY)) {
			DBG("%s(): invalid directory token. long_name = %s\n", __FUNCTION__, pdir_entry->long_name);
			dir_clus = INVALID_CLUSTER;
			dirent_release(pdir_entry);
			goto _release;
		}
	...
	IN	uint32 clus)
{
	...
	pdir->ptffs = ptffs;
	pdir->secbuf = (ubyte *)malloc(ptffs->pbs->byts_per_sec * ptffs->pbs->sec_per_clus);
