...
//	current_sa->esp_algo->iv = endian64(current_sa->esp_algo->iv);
	...
	if(DES_set_key_checked((unsigned char(*)[8])&(current_sa->esp_algo->esp_key[0]), &ks1) ||
...
//	memcpy(&packet->body[len], padding, pad_len);
	...
	switch(current_sp->mode)
	{
		case TRANSPORT :
			...
			break;
		case TUNNEL : 
			packet->body[len + pad_len + 1] = P_IP;
			break;
	}	
	// Padding Length Add
...
*/
	// 4. Encrypt Payload
//	memmove(packet->body, packet->body + sizeof(ESP), sizeof(ESP));
