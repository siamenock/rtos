...
/* PacketNgin modifcation */

static int __virtnet_poll(struct virtnet_info* vi)
{
	void *buf;
	unsigned int len, received = 0;
...
{
	struct dispatcher_work *work = (struct dispatcher_work *)data;
	struct virtnet_info *vi;
	if (!work->dev) {
		if (net_ratelimit())
			printk("ERROR: network device not registered yet\n");
		return ;
	}

	vi = netdev_priv(work->dev);
	__virtnet_poll(vi);
}
...
void dispatcher_tx(void *data)
{
	...
	while((packet = nic_process_output(0)) != NULL) {
		skb = convert_to_skb(vi->dev, packet);
		if(!skb) {
