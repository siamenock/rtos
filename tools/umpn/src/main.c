...
static int cmd_vm_create(int argc, char** argv, void(*callback)(char* result, int exit_status)) {
	...
	for(int i = 0; i < _vm->nic_count; i++) {
		/*
		if(vmspec->nics[i].mac == 0)
			vmspec->nics[i].mac = 0x998877665544 + i;
			*/
		uint64_t attrs[] = {
			NI_MAC, ((NEPort*)ne->nodes[i])->ti->mac,
			//NI_MAC, vmspec->nics[i].mac,
			NI_PORT, i,
...
int main(int _argc, char** _argv) { 
	...
	barrior_lock = tlsf_malloc(sizeof(uint8_t volatile));
	barrior = tlsf_malloc(sizeof(uint32_t volatile));
