...
HDD	:= -hda system.img
NIC	:= virtio #rtl8139
QEMU	:= qemu-system-x86_64 $(shell bin/qemu-params) -m 1024 -M pc -smp 8 -d cpu_reset -net nic,model=$(NIC) -net tap,script=bin/qemu-ifup -net nic,model=$(NIC) -net tap,script=bin/qemu-ifup $(USB) --no-shutdown --no-reboot  #$(HDD)
...
# Run by VirtualBox
ifeq ($(option),vb)
ifeq ("$(wildcard system.vdi)", "")
	VBoxManage convertfromraw system.img system.vdi
else
